...
class UserController extends Controller {
    ...
    public function home() {
        ...
        $data['deposit_balance']  = Deposit::where('user_id', Auth::id())->where('status', 1)->sum('amount');
        $data['withdraw_balance'] = Withdrawal::where('user_id', Auth::id())->where('status', 1)->sum('amount');
        $data['complete_job']     = JobProve::where('user_id', Auth::id())->where('status', 1)->count();
        $data['created_job']      = JobPost::where('user_id', Auth::id())->count();
        $data['transaction']      = Transaction::where('user_id', Auth::id())->count();
        $jobs                     = JobProve::where('user_id', Auth::id())->where('status', 1)->with('job', 'user')->latest()->take(5)->get();

        $jobProof = JobProve::select('id', 'created_at')
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $jobArr[$i]['month'] = $month[$i - 1];
        }

        return view($this->activeTemplate . 'user.dashboard', compact('pageTitle', 'data', 'jobs', 'emptyMessage', 'jobArr'));
    }
    ...
    public function submitProfile(Request $request) {
        ...
        $mobileNumber = User::where('id', '!=', $user->id)->where('mobile', $mobile)->exists();

        if ($mobileNumber) {
            $notify[] = ['success', 'Mobile number already exists.'];
    ...
    public function transaction() {
        ...
        $allTransaction = Transaction::where('user_id', Auth::id())->orderBy('id', 'desc')->paginate(getPaginate());
