...
class AdminController extends Controller {
    ...
    public function dashboard() {
        ...
        $job['total_post']     = JobPost::count();
        $job['ongoing_post']   = JobPost::where('status', 0)->count();
        $job['completed_post'] = JobPost::where('status', 2)->count();
        $job['cancel_post']    = JobPost::where('status', 3)->count();

        $job['total_balance']     = JobPost::sum('total');
        $job['ongoing_balance']   = JobPost::where('status', 0)->sum('total');
        $job['completed_balance'] = JobPost::where('status', 2)->sum('total');
        $job['cancel_balance']    = JobPost::where('status', 9)->sum('total');

        return view('admin.dashboard', compact('pageTitle', 'widget', 'report', 'withdrawals', 'chart', 'payment', 'paymentWithdraw', 'depositsMonth', 'withdrawalMonth', 'months', 'deposits', 'job'));
    }
