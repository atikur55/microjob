...
//For more, visit here http://fahad.thesoftking.com/tsk/tskadmin/docs-overlay.html
...
function loadCustomCaptcha($height = 46, $width = '100%', $bgcolor = '#003', $textcolor = '#ffffff') {
    ...
    $ret .= '<div style="height: ' . $height . 'px; line-height: ' . $height . 'px; width:' . $width . '; text-align: center; background-color: ' . $bgcolor . '; color: ' . $textcolor . '; font-size: ' . ($height - 20) . 'px; font-weight: bold; letter-spacing: 20px; font-family: \'Henny Penny\', cursive;  -webkit-user-select: none; -moz-user-select: none;-ms-user-select: none;user-select: none;  display: flex; justify-content: center; padding-top:10px;">';
...
function imagePath() {
    ...
    $data['job'] = [
        'attachment' => [
            'path' => 'assets/images/attachment/files',
            'size' => '550x400',
        ],
        'proof'      => [
            'path' => 'assets/images/attachment/proof',
        ],
...
function getContent($data_keys, $singleQuery = false, $limit = null, $orderById = false, $trxType = null) {
    ...
    if ($singleQuery) {
        ...
        if ($orderById) {
            $content = $article->where('data_keys', $data_keys)->orderBy('id');
        } else {
            $content = $article->where('data_keys', $data_keys)->orderBy('id', 'desc');
        }

        if ($trxType) {
            $content = $content->whereJsonContains('data_values->trx_type', $trxType);
        }
...
function countPost($id) {
    $categoryPost = JobPost::where('category_id', $id)->where('status', 1)->count();
    return $categoryPost;
}

function countSubcategory($id) {
    $subcategoryPost = JobPost::where('subcategory_id', $id)->where('status', 1)->count();
